...
This command will generate ``helloworld_pb2.py`` and ``helloworld_grpclib.py``
files.

Plugin, which implements ``--python_grpclib_out`` option is available for
``protoc`` compiler as ``protoc-gen-python_grpclib`` executable, which will be
installed by ``pip`` into your ``$PATH`` during installation of the
``grpclib`` library.
